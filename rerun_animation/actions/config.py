    ...
    match args.action:
        ...
        case "install":            
            ...
            if filename == Constants.DEFAULT_CONFIG_FILENAME:
                log.error(f":exclamation:\tCannot overwrite \"{Constants.DEFAULT_CONFIG_FILENAME}\", please rename the configuration.")
                exit(-1)
            else:
                dst = shutil.copy2(f"{name}.ini", os.path.join(configs_path, f"{name}.ini"))
