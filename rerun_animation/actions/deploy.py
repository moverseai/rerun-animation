...
def run():
    ...
    if (uses_external := args.deploy_path):
        ...
        out_folder = pkg_path
        # direct_url = Distribution.from_name("rerun-animation")#.read_text("direct_url.json")
    ...
    for gender_in, gender_out in zip(
        ...
        log.info(f":green_circle:\tDone. Saving to [italic]{output_filename}[/] ...")
        if uses_external:
    ...
    plugin_filename = f'{pkg_path}/rerun_animation/rerun-loader-animation.py'
    if platform.system() == 'Windows':
        ...
        for dll in dlls:
            shutil.copy2(dll, os.path.join(pkg_path, os.path.basename(dll)))
