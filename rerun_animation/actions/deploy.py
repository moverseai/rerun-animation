...
def run():
    ...
    log.debug(f"Setting up the `{'editable' if is_editable else 'installed'}` package @ {pkg_path}")
    ...
    log.info(f":gear:\tPreparing files from [italic]{os.path.abspath(data_root)}[/] to [italic]{os.path.abspath(out_folder)}[/].")
    ...
    for gender_in, gender_out in zip(
        ...
        data_files = glob.glob(
            os.path.join(data_root, f"**/basicmodel_{gender_in}*.pkl"), 
            recursive=True
        )
        if not data_files:
            # log.error(f":exclamation_mark:\tCould not locate body model data files @ [italic]{data_root}[/], deployment failed!")
            # sys.exit(-1)
            log.warning(f":exclamation_mark:\tCould not locate SMPL {gender_out} body model data files @ [italic]{data_root}[/]!")
        else:
            ...
            pose_blendshapes = np.ascontiguousarray(np.array(body_data['posedirs']).astype(np.float32))
            regressor = np.ascontiguousarray(np.array(body_data['J_regressor'].todense()).astype(np.float32))
            template = np.ascontiguousarray(np.array(body_data['v_template']).astype(np.float32))
            ...
            parents = np.ascontiguousarray(np.array(body_data['kintree_table']).astype(np.int32))[0]
            mesh = trimesh.Trimesh(vertices=template, faces=faces.astype(np.int64), process=False)
            ...
            np.savez_compressed(output_filename,
                ...
                regressor=regressor, template=template, weights=weights,
        ...
        if not data_files:
            # log.error(f":exclamation_mark:\tCould not locate body model data files @ [italic]{data_root}[/], deployment failed!")
            # sys.exit(-1)
            log.warning(f":exclamation_mark:\tCould not locate SMPL-H {gender_out} body model data files @ [italic]{data_root}[/]!")
        else:
            data_file = data_files[0]

            log.info(f"\t:orange_circle:Processing [italic]{data_file}[/] ...")
            body_data = np.load(data_file)
            shape_blendshapes = np.ascontiguousarray(body_data['shapedirs'].astype(np.float32))
            pose_blendshapes = np.ascontiguousarray(body_data['posedirs'].astype(np.float32))
            ...
            if uses_external:
                ...
                local_output_filename = os.path.join(pkg_path, f"SMPLH_{gender_out}.npz")
                #NOTE: this is not necessary as we are in control of these files
                #       it will only crash if the file was not symlinked and in use
                if os.path.exists(local_output_filename):
                    os.remove(local_output_filename)
    ...
    if platform.system() == 'Windows':
        ...
        with console.status("[bold green blink]...") as status:
            while process.poll() is None:
                time.sleep(1)
        log.info(f":green_circle:\tThe `rerun-loader-animation` plugin has been installed.")
        ...
        executable_filename = shutil.copy2(plugin_filename, os.path.join(pkg_path, os.path.basename(plugin_filename)))
        st = os.stat(executable_filename)
        os.chmod(executable_filename, st.st_mode | stat.S_IXUSR | stat.S_IXGRP | stat.S_IXOTH)
