...
from rerun_animation.bvh import (
    load as load_bvh,
...
def log_bvh(filename: str, entity_path: str, config) -> None:
    ...
    for i, t in enumerate(joint_positions):
        ...
        rr.log(f"{entity_path}/joints/positions", rr.Points3D(t, 
            class_ids=0, keypoint_ids=range(t.shape[0]), radii=joint_radius
        ))
        if show_rotations:
            rr.log(f"{entity_path}/joints/rotations", rr.Arrows3D(
...
def log_smpl_body(data_root: str, filepath: str, entity_path: str, config) -> None:
    ...
    body_data = np.load(os.path.join(data_root, f'{body_type.upper()}_{gender.upper()}.npz'))
    ...
    pose_type = config.get(f'{body_type}.options', 'pose_type', fallback=config.get('body', 'pose_type', fallback='axisangle'))
    pose = ensure_pose_shape(pose, pose_type)

    if orientation_key := config.get(f'{body_type}.file.keys', 'rotation', fallback=None):
    ...
    if body_type == 'smplh' and (hands_key := config.get(f'{body_type}.file.keys', 'hands', fallback=None)):
    ...
    translation_key = config.get(f'{body_type}.file.keys', 'translation', fallback='trans')
    ...
    if use_pose_blendshapes := config.getboolean(f'{body_type}.options', 'use_pose_blendshapes', fallback=False):
    ...
    if (fps_key := config.get(f'{body_type}.file.keys', 'fps', fallback=None)) and fps_key in animation_data:
    ...
    for i, rotation in enumerate(rotations):
        ...
        posed_joints, joint_transforms = traverse_kinematic_chain(rotation, shaped_joints, parents)
        posed_vertices, posed_normals = skinning(weights, vertices, joint_transforms, normals)

        rr.set_time_seconds("time", i * delta_time_secs)
        rr.log(f"/{entity_path}/joints", rr.Points3D(posed_joints + translation[i], 
...
def get_args() -> None:
    ...
    parser.add_argument("--time", type=str, action="append", help="optional timestamps to log at (e.g. `--time sim_time=1709203426`)")
    parser.add_argument("--sequence", type=str, action="append", help="optional sequences to log at (e.g. `--sequence sim_frame=42`)")
