...
def forward_kinematics(
    ...
    if parents.shape[-1] == offset.shape[0]: #NOTE: to support using the same parents everywhere
        parents = parents[1:]
    for current_idx, parent_idx in enumerate(parents, start=1): #NOTE: assumes parents exclude root
        joints[..., current_idx, :] = \
            np.einsum('brc,c -> br', global_rotation[..., parent_idx, :, :], offset[current_idx])
